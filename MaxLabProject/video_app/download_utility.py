...
class download_utility:
    ...
    def __init__(self, room_id, type=None, no_of_participants=None, force_download=False):
    ...
    def getZipFile(self):
        ...
        if os.path.isfile(f"{self.path}/{self.room_id}_{self.text_type}.zip"):
            with ZipFile(f"{self.path}/{self.room_id}_{self.text_type}.zip", 'r') as zipObj:
                listOfiles = zipObj.namelist()
                print(len(listOfiles), len(fileList), self.no_of_participants)
                if(len(listOfiles) >= self.no_of_participants):
                    print("inside")
                    return None, f"{self.path}/{self.room_id}_{self.text_type}.zip", f"{self.room_id}_{self.text_type}.zip"

                if(len(listOfiles) >= len(fileList) and len(fileList) >= self.no_of_participants):
                    print("inside2")
                    return None, f"{self.path}/{self.room_id}_{self.text_type}.zip", f"{self.room_id}_{self.text_type}.zip"
        ...
        if len(fileList) < self.no_of_participants and self.force_download is False:
            return f"Not all the {self.text_type} are present for the room {self.room_id}. Do you wish to proceed with download ?", None, None
